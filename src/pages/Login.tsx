...
import { authenticate, getStoredSession, storeSession, SUPER_ADMIN_CREDENTIALS } from "@/lib/auth";
    ...
    window.setTimeout(() => {
      ...
      if (!session) {
        toast({
          ...
          description: "Verifica el usuario y la contraseña del super admin.",
          variant: "destructive",
        });
        return;
      }
      ...
      toast({
        title: "Bienvenido",
        description: "Sesion iniciada como super admin.",
      });
      navigate("/mapa", { replace: true });
    }, 700);
  };
    ...
    >
      ...
      >
        ...
        >
          <div className="pt-10 pb-6 px-8 text-center">
            ...
            >
              Accede al Sistema de Informacion
              <br />
              Geografica de Ocana
            </motion.p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="px-8 pb-6 space-y-4">
            ...
            >
              ...
              <Input
                type="text"
                placeholder="superadmin o superadmin@sigocana.local"
                value={email}
          ...
          <div className="px-8 pb-7">
            <div className="rounded-2xl border border-[#4a7c59]/15 bg-[#4a7c59]/6 px-4 py-3">
              <p className="text-[11px] font-bold uppercase tracking-wider text-[#4a7c59]">
                Credenciales de acceso
              </p>
              <p className="text-[12px] text-[#6b5b4e] mt-2">
                Usuario: <span className="font-semibold">{SUPER_ADMIN_CREDENTIALS.username}</span>
              </p>
              <p className="text-[12px] text-[#6b5b4e]">
                Correo: <span className="font-semibold">{SUPER_ADMIN_CREDENTIALS.email}</span>
              </p>
              <p className="text-[12px] text-[#6b5b4e]">
                Contrasena: <span className="font-semibold">{SUPER_ADMIN_CREDENTIALS.password}</span>
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="absolute bottom-4 text-center w-full z-10">
        <p className="text-[10px] text-[#b4a898] font-medium">
          © 2026 Alcaldia de Ocana · Norte de Santander, Colombia
        </p>
      </div>
    </div>
  );
